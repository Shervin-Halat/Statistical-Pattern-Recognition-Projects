...
for cls in classes:
    ...
    for datapoint in ts_proj[cls]:
        sign = datapoint[1]-((datapoint[0] - miu_mean[0])*(-v[0]/v[1]) + miu_mean[1])
        if sign > minsign and cls == 0:
            min_count[cls] = counter
        elif sign < minsign and cls == 1:
            min_count[cls] = counter
        counter += 1
        
for i in range(2):
    img = ts_data[i][min_count[i]]
    plt.imshow(img.reshape((28,28)) , cmap = 'gray')
    plt.title('most incorrectly classified sample of {}'.format(new_labels[i]))
    plt.show()
