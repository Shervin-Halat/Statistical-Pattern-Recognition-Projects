...
for value in k:
    num = k.index(value)
    plt.subplot2grid((2,2),(num//2, num%2))
    b = vec[np.where(np.isclose(val,val_sort[value]))[0][0]]
    c = (b.T).dot(im_st)
    d = np.array([b*i for i in c])
    d = d.T 
    e = np.array([d[:,i].reshape(8,8) for i in range (d.shape[1])])
    z = unblockshaped(e, 360, 240)
    plt.imshow(z,'gray')
    plt.title('reconstructed subimage of k value = {}'.format(value))
plt.suptitle('reconstructed subimage of single eigenvector(k value)')
plt.show() 
