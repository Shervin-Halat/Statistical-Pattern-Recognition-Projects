...
for i in range(len(x_test)):
    if y_test[i] == 2:
        ...
        c = 1
    ts_data[c].append(np.concatenate(x_test[i])) #


for i in range(2):
    tr_data[i] = np.array(tr_data[i])
    ts_data[i] = np.array(ts_data[i]) ##

x_train_col = np.array([np.concatenate(i) for i in x_train])
x_test_col = np.array([np.concatenate(i) for i in x_test]) ###
...
for cl in new_classes:
    ...
    x_ts_proj = np.array([[v.dot(point) for v in eig_vec] for point in ts_data[cl]])###
    tr_proj[num] = x_proj
    ts_proj[num] = x_ts_proj ###
...
classes = [0,1]
for cls in classes:
    for datapoint in ts_proj[cls]:
        sign = datapoint[1]-((datapoint[0] - miu_mean[0])*(-v[0]/v[1]) + miu_mean[1])
        if sign <= 0:
            column = 0
        else:
            column = 1
        con_mat_test[cls][column] += 1
accuracy = 100*(sum([con_mat_test[i,i] for i in range(len(con_mat_test))]) / con_mat_test.sum())
print('confusion matrix for test set would be:\n',con_mat_test)
print('accuracy for test set would be:\n','{:.2f}'.format(accuracy))
